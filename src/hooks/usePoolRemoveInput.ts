...
export const useMultiPoolRemoveInput = () => {
  ...
  const bins = useAppSelector((state) => state.pools.selectedBins);
  const binDecimals = bins.length > 0 ? bins[0].clbTokenDecimals : CLB_TOKEN_VALUE_DECIMALS;
  const { clbTokenBalance, liquidityValue, removableLiquidity } = useMemo(() => {
    return bins.reduce(
      (record, bin) => {
        const { clbTokenBalance, clbTokenValue, freeLiquidity } = bin;
        const liquidityValue = clbTokenBalance.mul(clbTokenValue);
        return {
