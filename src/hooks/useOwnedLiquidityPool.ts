...
export const useOwnedLiquidityPool = () => {
  ...
  const fetchKey = isValid(address) && isValid(token) && isValid(markets) ? [
    address, token.address, ...markets.map(market => market.address)
  ] : undefined

  const {data: ownedPools, error, isLoading, mutate: fetchOwnedPools} = useSWR(fetchKey, async ([address, tokenAddress, ...marketAddresses]) => {
    if (!isValid(client)) {
      return
    }
