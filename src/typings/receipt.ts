...
export class LPReceipt {
  ...
  constructor(output: BaseOutput) {
    const { id, oracleVersion, amount, action, recipient, tradingFeeRate } = output;
    this.id = id;
  ...
  updateIsCompleted(currentVersion: bigint) {
    const isCompleted = this.version < currentVersion;
  ...
  renderName(market: Market) {
    ...
    return (
      ...
      ' ' +
      prefix +
      ((this.feeRate * percentage()) / numberBuffer(FEE_RATE_DECIMAL)).toFixed(2) +
      '%'
    );
  }
