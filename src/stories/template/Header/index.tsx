          ...
          {isValid(account?.walletAddress) ? (
            ...
            <>
              <button
                onClick={onConnect}
                title="connect"
                className="p-[2px] flex items-center border rounded-full bg-black border-grayL text-white min-w-[175px]"
              >
                <Avatar src={arbitrumIcon} className="!w-[36px] !h-[36px]" />
                <p className="w-full pr-5 text-lg font-normal text-center">Connect</p>
              </button>
            </>
          )}
        </div>
